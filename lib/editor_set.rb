  ...
  class EditorSet < KDE::TabWidget
    ...
    def initialize(parent)
      ...
      connect(self, SIGNAL("contextMenu(QWidget*, const QPoint&)")) do |editor, pos|
        ...
        Kodr::App.instance.gui_factory.container("tabContextMenu", Kodr::App.instance).exec(pos)
    ...
    def setup_tab_close_button
      ...
      connect(close_button, SIGNAL("clicked()")) do
        Kodr::App.instance.action_collection.action("file_close").trigger
      end
    end
    
    def open_url(url, line=nil)
      url ||= KDE::Url.new("")
      if !url.is_empty && editor = get_editor_for_url(url)
        set_current_widget(editor)
      else
        if editors.size == 1 && active_editor.url.is_empty && !active_editor.document.is_modified && !url.is_empty
      ...
      if line
        editor.set_cursor_position(KTextEditor::Cursor.new(line-1, 0))
      end
      editor
    end
    
    def create_document(url)
      doc = TextDocument.new #KTextEditor::EditorChooser::editor.create_document(nil)
      if url.is_empty
        return doc
      end
      if doc.open_url(url)
...
#       GC.start
    ...
    def get_editor_for_url(url)
      editors.detect { |e| e.url == url }
    end
